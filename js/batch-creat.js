...
var proInfo = new Vue({
	...
	computed: {
		...
		url:function(){
			return 'batch-list.html?id=' + this.textInfo.id+ '&category_id=' + this.textInfo.category_id;
		}
	},
	methods:{
		//提交数据
		submit:function(){
			if (!$('.product-text .save-text').val()) {
				layer.msg('请输入内容');
				...
				$.ajax({
				    type: "POST",
				    url: "http://192.168.1.42/canton/index.php/post/batchinfo", //添加请求地址的参数
				    dataType: "json",
				    data:{
